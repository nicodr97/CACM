...
class Main
{
	...
	public static void main( String args[] )
	{
		...
		while( ++escenario <= escenarios ) {
            ...
            int erdnum = 0;
            erdos.setErdosNumber(erdnum);
            LinkedList<Autor> open = new LinkedList<Autor>();
            open.add(erdos);
            while(!open.isEmpty()) {
                Autor aut = open.pop();
                erdnum = aut.getErdosNumber()+1;
                LinkedList<Autor> coaut = aut.getCoautores();
                int num_coaut = coaut.size();
                for(int i = 0; i < num_coaut; i++) {
                    aut = coaut.get(i);
                    if(!aut.getEnCola()) {
                        aut.setErdosNumber(erdnum);
                        open.add(aut);
                        aut.setEnCola(true);
                    }
                }
